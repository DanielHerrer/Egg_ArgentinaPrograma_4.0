Algoritmo vectorPosicionTeclaCambiar 
	...
	Mientras Que Longitud(frase)>20 		/// SE ASEGURA DE QUE LA FRASE SEA DE 20 CARACTERES O MENOS
	...
	Si vector(posicion-1) <> "" Y vector(posicion-1) <> " " Entonces
		
		Si vector(19) == "" O vector(19) == " " Entonces
			Escribir "No hay lugar en la posicion, reacomodando vector... "
			Para j<-18 Hasta posicion-1 Con Paso -1 Hacer
				vector(j+1) = vector(j)
			FinPara
			vector(posicion-1) = letra
		SiNo
			Escribir "No hay lugar en la posicion y tampoco en el vector."
		FinSi
